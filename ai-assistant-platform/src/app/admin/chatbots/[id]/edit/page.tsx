/**
 * Edit Chatbot Page
 * Form for editing an existing chatbot with extended model parameters
 */
...
export default function EditChatbotPage() {
  ...
  const fetchChatbot = useCallback(async () => {
    ...
    try {
      ...
      // Extract settings
      const modelParams = chatbot.settings?.model_params || {};
      ...
      setFormData({
        ...
        top_p: modelParams.top_p ?? 1,
        frequency_penalty: modelParams.frequency_penalty ?? 0,
        presence_penalty: modelParams.presence_penalty ?? 0,
        // Provider options
        reasoning_effort: providerOptions.reasoning_effort ?? 'medium',
        text_verbosity: providerOptions.text_verbosity ?? 'medium',
        store_completions: providerOptions.store ?? false,
        // Publishing
        is_published: chatbot.is_published,
      });

      // Show advanced section if any advanced parameters are non-default
      const hasAdvancedSettings =
        (modelParams.frequency_penalty !== undefined && modelParams.frequency_penalty !== 0) ||
        (modelParams.presence_penalty !== undefined && modelParams.presence_penalty !== 0) ||
        providerOptions.store === true ||
        providerOptions.reasoning_effort !== undefined ||
        providerOptions.text_verbosity !== undefined;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({
        general: err instanceof Error ? err.message : 'Failed to update chatbot',
      });
    } finally {
      setSaving(false);
    }
      ...
      <div className="p-6 lg:p-8">
        ...
        <div className="mx-auto max-w-2xl">
          <div className="flex flex-col items-center justify-center rounded-xl border border-red-200 bg-red-50 py-12 dark:border-red-800 dark:bg-red-900/20">
            ...
            <p className="mt-4 text-red-700 dark:text-red-400">{fetchError}</p>
            <Button
              variant="outline"
